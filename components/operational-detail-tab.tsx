    ...
    return (
      <div className="flex flex-col items-center justify-center h-80 text-center space-y-4">
        ...
        <div>
          ...
          <p className="text-on-surface-variant">Faça upload dos dados do KMM para navegar pelas rotas.</p>
        </div>
      </div>
    )
  }
    ...
    <div className="space-y-lg text-on-surface">
      ...
      <section className="bg-surface-container-lowest rounded-xl p-md shadow-surface flex flex-col gap-md border border-outline-variant/30">
        <div className="flex items-center justify-between pb-sm border-b border-outline-variant/20">
          <h2 className="font-headline-md text-headline-md text-on-surface flex items-center gap-sm tracking-tight">
            <span className="material-symbols-outlined text-primary">filter_list</span>
            Filtros de Operação
          </h2>
          <button 
            onClick={() => { setFilters(INITIAL_FILTERS); setSearchGlobal(''); setCurrentPage(1); }}
            className="text-secondary hover:text-primary font-label-lg text-label-lg flex items-center gap-xs transition-colors font-bold uppercase tracking-wider text-[10px]"
          >
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-7 gap-md">
          ...
          {/* Célula */}
          <div className="flex flex-col gap-xs">
            <label className="font-label-md text-label-md text-on-surface-variant uppercase tracking-tighter font-bold text-[10px]">Célula</label>
          ...
          {/* Status */}
          <div className="flex flex-col gap-xs">
            <label className="font-label-md text-label-md text-on-surface-variant uppercase tracking-tighter font-bold text-[10px]">Status</label>
          ...
          <div className="flex flex-col gap-xs xl:col-span-3">
            <label className="font-label-md text-label-md text-on-surface-variant uppercase tracking-tighter font-bold text-[10px]">Busca Rápida</label>
        ...
        <button 
          ...
          className="text-[11px] text-primary font-bold hover:underline text-left w-fit uppercase tracking-wider"
        >
        ...
        {showAdvancedFilters && (
          <div className="grid grid-cols-1 md:grid-cols-4 gap-md pt-md border-t border-outline-variant/10">
            <div className="flex flex-col gap-xs">
              <label className="font-label-md text-[10px] text-on-surface-variant uppercase font-bold">Planta</label>
            ...
            <div className="flex flex-col gap-xs">
              <label className="font-label-md text-[10px] text-on-surface-variant uppercase font-bold">Status KM</label>
              <Select value={filters.kmStatus} onValueChange={(v) => { setFilters(p => ({ ...p, kmStatus: v as any })); setCurrentPage(1) }}>
                <SelectTrigger className="h-9 bg-surface text-[11px]">
                  <SelectValue placeholder="Todos os Status KM" />
                </SelectTrigger>
                <SelectContent>
                  <SelectItem value="all">Todos os Status KM</SelectItem>
            ...
            <div className="flex flex-col gap-xs">
              <label className="font-label-md text-[10px] text-on-surface-variant uppercase font-bold">Placa</label>
            ...
            <div className="flex flex-col gap-xs">
              <label className="font-label-md text-[10px] text-on-surface-variant uppercase font-bold">Nº Roteiro</label>
              <Input 
                placeholder="Roteiro..." 
      ...
      <section className="bg-surface-container-lowest rounded-xl shadow-surface flex flex-col flex-1 overflow-hidden border border-outline-variant/30">
        <div className="h-14 px-md flex items-center justify-between border-b border-outline-variant/20 bg-surface-bright/30 shrink-0">
          <div className="flex items-center gap-sm">
            <h3 className="font-headline-md text-headline-md text-on-surface tracking-tight font-bold">Detalhamento Operacional</h3>
          ...
          <div className="text-[10px] text-on-surface-variant uppercase font-bold tracking-tighter">
            Extração: {formatDateBR(auditPeriod.start)}
          </div>
        </div>
          ...
          <table className="w-full text-left border-collapse min-w-[1200px]">
            <thead className="sticky top-0 bg-surface-container-lowest shadow-[0_1px_0_0_rgba(0,0,0,0.05)] z-10">
              <tr className="bg-surface-container-low/50">
                <th className="h-12 px-md font-label-lg text-label-lg text-on-surface-variant whitespace-nowrap font-bold text-[10px] uppercase">Cél.</th>
                <th className="h-12 px-md font-label-lg text-label-lg text-on-surface-variant whitespace-nowrap font-bold text-[10px] uppercase">Planta</th>
                <th className="h-12 px-md font-label-lg text-label-lg text-on-surface-variant whitespace-nowrap font-bold text-[10px] uppercase">Roteiro</th>
                <th className="h-12 px-md font-label-lg text-label-lg text-on-surface-variant whitespace-nowrap font-bold text-center text-[10px] uppercase">Data Rota</th>
                <th className="h-12 px-md font-label-lg text-label-lg text-on-surface-variant whitespace-nowrap font-bold text-center text-[10px] uppercase">Status</th>
                <th className="h-12 px-md font-label-lg text-label-lg text-on-surface-variant whitespace-nowrap font-bold text-center text-[10px] uppercase">KM Status</th>
                <th className="h-12 px-md font-label-lg text-label-lg text-on-surface-variant text-right whitespace-nowrap font-bold text-[10px] uppercase">L. Col.</th>
                <th className="h-12 px-md font-label-lg text-label-lg text-on-surface-variant text-right whitespace-nowrap font-bold text-[10px] uppercase">L. Des.</th>
                <th className="h-12 px-md font-label-lg text-label-lg text-on-surface-variant whitespace-nowrap font-bold text-[10px] uppercase">Placa</th>
                <th className="h-12 px-md font-label-lg text-label-lg text-on-surface-variant whitespace-nowrap font-bold text-[10px] uppercase">Observação</th>
              </tr>
            </thead>
            <tbody className="font-body-sm text-body-sm text-on-surface divide-y divide-outline-variant/10">
              {paginatedRoutes.map((r, idx) => (
                <tr key={r.id} className={`h-[52px] transition-colors hover:bg-primary/5 group ${idx % 2 !== 0 ? 'bg-surface-bright/10' : ''} ${getRowClassName(r)}`}>
                  ...
                  <td className="px-md font-mono text-[11px] font-bold">{r.roteiro}</td>
                  <td className="px-md text-center whitespace-nowrap font-medium text-on-surface-variant">{formatDateBR(r.dataRota)}</td>
                  ...
                  <td className="px-md text-center">
                    <span className={`font-bold text-[11px] ${r.kmStatus !== 'OK' ? 'text-orange' : 'text-success/80'}`}>
                      {r.kmStatus}
                    </span>
                  </td>
        ...
        <div className="h-14 px-md flex items-center justify-between border-t border-outline-variant/20 bg-surface-container-low/30 shrink-0">
          <span className="font-body-sm text-[11px] text-on-surface-variant font-bold uppercase tracking-wider">Mostrando {paginatedRoutes.length} de {filteredRoutes.length} rotas filtradas</span>
          <div className="flex items-center gap-sm">
            <Button size="sm" variant="ghost" disabled={currentPage === 1} onClick={() => setCurrentPage(p => p - 1)} className="h-8 w-8 p-0 hover:bg-primary/10">
              <span className="material-symbols-outlined">chevron_left</span>
            </Button>
            <div className="flex items-center gap-1 font-mono text-xs">
              <span className="bg-primary text-on-primary px-2 py-0.5 rounded font-bold shadow-sm">{currentPage}</span>
              <span className="text-on-surface-variant font-bold">/</span>
              <span className="text-on-surface-variant font-bold">{totalPages || 1}</span>
            </div>
            <Button size="sm" variant="ghost" disabled={currentPage === totalPages || totalPages === 0} onClick={() => setCurrentPage(p => p + 1)} className="h-8 w-8 p-0 hover:bg-primary/10">
      ...
      <div className="p-lg rounded-xl bg-surface-container-low/50 border border-outline-variant/20 space-y-md">
         ...
         <div className="grid grid-cols-1 md:grid-cols-3 gap-lg">
            <div className="flex items-start gap-3 p-md rounded-lg bg-surface-container-lowest border border-outline-variant/10 shadow-sm">
               ...
               <div>
                  <p className="font-bold text-xs text-error uppercase tracking-tight">Rota Crítica</p>
                  <p className="text-[10px] text-on-surface-variant font-medium leading-tight mt-1">Status Pendente + Divergência de Leite (SCL), KM Incorreto ou Regresso Antigo.</p>
               </div>
            </div>
            <div className="flex items-start gap-3 p-md rounded-lg bg-surface-container-lowest border border-outline-variant/10 shadow-sm">
               <div className="w-4 h-full bg-error-container/10 shrink-0 rounded" />
               <div>
                  <p className="font-bold text-xs text-on-surface-variant uppercase tracking-tight">Pendente / Em Execução</p>
                  <p className="text-[10px] text-on-surface-variant font-medium leading-tight mt-1">Rota em andamento no KMM sem outras inconsistências críticas identificadas.</p>
               </div>
            </div>
            <div className="flex items-start gap-3 p-md rounded-lg bg-surface-container-lowest border border-outline-variant/10 shadow-sm">
               <div className="w-4 h-full bg-warning/10 shrink-0 rounded" />
               <div>
                  <p className="font-bold text-xs text-warning uppercase tracking-tight">S.C. Leite (Encerrada)</p>
                  <p className="text-[10px] text-on-surface-variant font-medium leading-tight mt-1">Rota encerrada no KMM porém sem registro de litros descarregados (Canhoto).</p>
               </div>
            </div>
         </div>
      </div>
